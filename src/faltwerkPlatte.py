...
def main():
   ...
   filename = "C:/FG/feap/exe/rve/microShell/Faltwerke/iPlatte"
   
   # parameters
   li = 0      # linear geometry?  0=lin,1=nili (moderate),2=nili (finite) 
   t = 0.05    # shell thickness
   e = 7000    # Young's modulus
   v = 0.34    # Poisson ratio
   f = -0.003  # loading force
...
#    nmnbnz = '000'
   ...
   nx = 8 # nr. of vertically aligned shells in y direction (x=const.)
   ny = 8 # nr. of vertically aligned shells in x direction (y=const.)
   ...
   hx = 8 # elements between two vertical shells (x direction) // total elements in x-direction if nx <= 1
   hy = 8 # elements between two vertical shells (y direction) // total elements in y-direction if ny <= 1
   
   hz = 4 # elements in thickness direction
   ...
   if top: topBloc = Block2d(4, hx*nx if nx > 0 else hx, hy*ny if ny > 0 else hy, NODE_COUNTER, ELEM_COUNTER, 2, 
   ...
   # schräge Ansicht
   footer = Footer(tie=True, macr=('prop', 'plot,rot1,-60', 'plot,rot3,30', 'plot,mesh', 'plot,axis', 'plot,boun,,1', 'plot,boun,,2', 'plot,boun,,3', 'plot,node,,-%i' % tploNode,
                                   'tplo,init,%i,-3,1' % tploNode, 'tplo,mark,2,4,1'), macrBlanks=1 )
   ...
   for ix in range(nx):
      ...
      vxBlocs.append(  Block2d(4, hy*ny if ny>0 else hy, hz, NODE_COUNTER, ELEM_COUNTER, 3, 
   ...
   for iy in range(ny):
      ...
      vyBlocs.append( Block2d(4, hx*nx if nx>0 else hx, hz, NODE_COUNTER, ELEM_COUNTER, 3, 
